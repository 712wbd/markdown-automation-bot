...
function displayBanner() {
  ...
  console.log(
    boxen(
      chalk2.white.bold("\u{1F916} Enterprise Markdown Automation Bot\n\n") + chalk2.gray("Generate professional Markdown files with ease\n") + chalk2.gray("Supports GitHub, Documentation, API Docs, and more"),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round",
...
program.command("info <template-id>").description("Show detailed information about a template").action(async (templateId) => {
  ...
  console.log(
    boxen(
      chalk2.cyan.bold(template.name) + "\n\n" + chalk2.white(template.description) + "\n\n" + chalk2.gray(`ID: ${template.id}`) + "\n" + chalk2.gray(`Category: ${template.category}`) + "\n" + chalk2.gray(`Tags: ${template.tags.join(", ")}`),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round"
  ...
  logger2.info(`
${chalk2.bold("Required Fields:")}`);
  const requiredFields = template.fields.filter((f) => f.required);
  requiredFields.forEach((field) => {
    console.log(`  ${chalk2.green("\u2022")} ${chalk2.white(field.label)} ${chalk2.gray(`(${field.type})`)}`);
    if (field.placeholder) {
      console.log(`    ${chalk2.gray("Placeholder:")} ${field.placeholder}`);
    }
  });
  const optionalFields = template.fields.filter((f) => !f.required);
  if (optionalFields.length > 0) {
    logger2.info(`
${chalk2.bold("Optional Fields:")}`);
    optionalFields.forEach((field) => {
      console.log(`  ${chalk2.yellow("\u2022")} ${chalk2.white(field.label)} ${chalk2.gray(`(${field.type})`)}`);
    });
  }
  console.log();
});
program.command("categories").alias("cat").description("List all template categories").action(async () => {
  ...
  const table = new Table({
    head: [chalk2.cyan("Category"), chalk2.cyan("Count")],
    colWidths: [30, 10]
  });
...
`);
  matches.forEach((match) => {
    console.log(
      `${chalk2.green("\u2022")} ${chalk2.white.bold(match.template.name)} ` + chalk2.gray(`[${match.template.id}]`) + ` ${chalk2.yellow(`(score: ${match.score})`)}`
    );
    console.log(`  ${chalk2.gray(match.template.description)}`);
    console.log(`  ${chalk2.cyan("Matched:")} ${match.matchedFields.join(", ")}
`);
  });
});
program.command("stats").description("Show statistics about templates").action(async () => {
  ...
  console.log(
    boxen(
      chalk2.cyan.bold("\u{1F4CA} Template Statistics\n\n") + chalk2.white(`Total Templates: ${stats.total}
`) + chalk2.white(`Total Fields: ${stats.totalFields}
`) + chalk2.white(`Total Sections: ${stats.totalSections}`),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round"
...
async function interactiveGenerate(options) {
  ...
  logger2.info(`
${chalk2.bold("Template:")} ${template.name}`);
  logger2.info(`${chalk2.gray(template.description)}
`);
  const fields = {};
  for (const field of template.fields) {
    const prompt = {
      name: field.name,
      message: field.label + (field.required ? chalk2.red(" *") : "")
    };
    switch (field.type) {
      case "text":
      case "email":
      ...
      case "array":
        prompt.type = "input";
        prompt.message += chalk2.gray(" (comma-separated)");
...
\u2713 File generated successfully!`);
      console.log(
        boxen(
          chalk2.white.bold("Generation Summary\n\n") + chalk2.gray(`File: ${result.filePath}
`) + chalk2.gray(`Size: ${result.metadata.fileSize} bytes
`) + chalk2.gray(`Lines: ${result.metadata.lineCount}`),
          {
            padding: 1,
            margin: 1,
            borderStyle: "round",
