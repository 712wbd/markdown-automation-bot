...
var FileGenerator = class {
  ...
  templateEngine;
  templateLibrary;
  constructor(fileSystem2, templateEngine2, templateLibrary2) {
    this.logger = new Logger("FileGenerator");
    this.fileSystem = fileSystem2;
    this.templateEngine = templateEngine2;
    this.templateLibrary = templateLibrary2;
  }
...
var logger2 = new Logger("CLI");
var fileSystem = new FileSystemManager();
var templateEngine = new TemplateEngine();
var templateLibrary = new TemplateLibrary();
var fileGenerator = new FileGenerator(fileSystem, templateEngine, templateLibrary);
for (const template of allTemplates) {
  templateLibrary.registerTemplate(template);
}
function displayBanner() {
  console.clear();
  const banner = import_figlet.default.textSync("MarkdownBot", {
    font: "Standard",
  ...
  console.log(
    (0, import_boxen.default)(
      import_chalk2.default.white.bold("\u{1F916} Enterprise Markdown Automation Bot\n\n") + import_chalk2.default.gray("Generate professional Markdown files with ease\n") + import_chalk2.default.gray("Supports GitHub, Documentation, API Docs, and more"),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round",
...
program.name("mdbot").description("\u{1F916} Enterprise Markdown Automation Bot").version("1.0.0");
program.command("generate").alias("gen").alias("g").description("Generate a Markdown file from a template").option("-t, --template <id>", "Template ID").option("-c, --category <category>", "Template category").option("-o, --output <path>", "Output directory path", process.cwd()).option("-f, --filename <name>", "Output filename").option("--no-validate", "Skip Markdown syntax validation").option("--no-backup", "Do not create backup of existing files").option("--dry-run", "Preview output without writing file").option("-i, --interactive", "Interactive mode").action(async (options) => {
  try {
    displayBanner();
    ...
    logger2.error("Generation failed:", error);
    process.exit(1);
  }
});
program.command("list").alias("ls").description("List all available templates").option("-c, --category <category>", "Filter by category").option("-t, --tag <tag>", "Filter by tag").option("--json", "Output as JSON").action(async (options) => {
  displayBanner();
  let templates = templateLibrary.getAllTemplates();
  if (options.category) {
    templates = templateLibrary.getTemplatesByCategory(options.category);
  }
  if (options.tag) {
  ...
  if (templates.length === 0) {
    logger2.warn("No templates found matching the criteria");
    return;
  }
  const table = new import_cli_table3.default({
    head: [
      import_chalk2.default.cyan("ID"),
      import_chalk2.default.cyan("Name"),
      import_chalk2.default.cyan("Category"),
      import_chalk2.default.cyan("Tags")
    ],
  ...
  console.log("\n" + table.toString() + "\n");
  logger2.info(`Total templates: ${templates.length}`);
});
program.command("info <template-id>").description("Show detailed information about a template").action(async (templateId) => {
  displayBanner();
  const template = templateLibrary.getTemplate(templateId);
  if (!template) {
    logger2.error(`Template not found: ${templateId}`);
    process.exit(1);
  }
  console.log(
    (0, import_boxen.default)(
      import_chalk2.default.cyan.bold(template.name) + "\n\n" + import_chalk2.default.white(template.description) + "\n\n" + import_chalk2.default.gray(`ID: ${template.id}`) + "\n" + import_chalk2.default.gray(`Category: ${template.category}`) + "\n" + import_chalk2.default.gray(`Tags: ${template.tags.join(", ")}`),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round"
  ...
  logger2.info(`
${import_chalk2.default.bold("Required Fields:")}`);
  const requiredFields = template.fields.filter((f) => f.required);
  requiredFields.forEach((field) => {
    console.log(`  ${import_chalk2.default.green("\u2022")} ${import_chalk2.default.white(field.label)} ${import_chalk2.default.gray(`(${field.type})`)}`);
    if (field.placeholder) {
      console.log(`    ${import_chalk2.default.gray("Placeholder:")} ${field.placeholder}`);
    }
  });
  const optionalFields = template.fields.filter((f) => !f.required);
  if (optionalFields.length > 0) {
    logger2.info(`
${import_chalk2.default.bold("Optional Fields:")}`);
    optionalFields.forEach((field) => {
      console.log(`  ${import_chalk2.default.yellow("\u2022")} ${import_chalk2.default.white(field.label)} ${import_chalk2.default.gray(`(${field.type})`)}`);
    });
  }
  console.log();
});
program.command("categories").alias("cat").description("List all template categories").action(async () => {
  displayBanner();
  ...
  const table = new import_cli_table3.default({
    head: [import_chalk2.default.cyan("Category"), import_chalk2.default.cyan("Count")],
    colWidths: [30, 10]
  });
...
program.command("validate <file>").description("Validate a Markdown file for syntax errors").action(async (file) => {
  displayBanner();
  const spinner = (0, import_ora.default)("Reading file...").start();
  try {
    ...
    spinner.stop();
    if (result.valid) {
      logger2.success("\u2713 Markdown file is valid!");
    } else {
      logger2.error("\u2717 Markdown file has errors:");
      result.errors.forEach((e) => console.log(`  ${import_chalk2.default.red("\u2022")} ${e}`));
    }
    if (result.warnings.length > 0) {
      logger2.warn("Warnings:");
      result.warnings.forEach((w) => console.log(`  ${import_chalk2.default.yellow("\u2022")} ${w}`));
    }
    console.log(`
...
program.command("batch <config-file>").description("Generate multiple files from a configuration file").option("-o, --output <path>", "Output directory", process.cwd()).action(async (configFile, options) => {
  displayBanner();
  try {
    logger2.info(`Loading batch configuration: ${configFile}`);
    const config = await import(path4.resolve(configFile));
    const inputs = config.default || config.inputs;
    if (!Array.isArray(inputs) || inputs.length === 0) {
      logger2.error("Invalid batch configuration");
      process.exit(1);
    }
    logger2.info(`Processing ${inputs.length} files...`);
...
Batch generation completed successfully!`);
    ...
    logger2.error("Batch generation failed:", error);
    process.exit(1);
  }
});
program.command("init").description("Initialize a new MarkdownBot configuration").option("-d, --dir <path>", "Directory to initialize", process.cwd()).action(async (options) => {
  displayBanner();
  ...
  const configPath = path4.join(options.dir, ".mdbotrc.json");
  const config = {
  ...
  logger2.success(`Configuration created: ${configPath}`);
});
program.command("search <query>").description("Search for templates by keyword").action(async (query) => {
  displayBanner();
  ...
  if (matches.length === 0) {
    logger2.warn(`No templates found matching: ${query}`);
    return;
  }
  logger2.info(`Found ${matches.length} matching templates:
`);
  matches.forEach((match) => {
    console.log(
      `${import_chalk2.default.green("\u2022")} ${import_chalk2.default.white.bold(match.template.name)} ` + import_chalk2.default.gray(`[${match.template.id}]`) + ` ${import_chalk2.default.yellow(`(score: ${match.score})`)}`
    );
    console.log(`  ${import_chalk2.default.gray(match.template.description)}`);
    console.log(`  ${import_chalk2.default.cyan("Matched:")} ${match.matchedFields.join(", ")}
`);
  });
});
program.command("stats").description("Show statistics about templates").action(async () => {
  displayBanner();
  const stats = templateLibrary.getStatistics();
  console.log(
    (0, import_boxen.default)(
      import_chalk2.default.cyan.bold("\u{1F4CA} Template Statistics\n\n") + import_chalk2.default.white(`Total Templates: ${stats.total}
`) + import_chalk2.default.white(`Total Fields: ${stats.totalFields}
`) + import_chalk2.default.white(`Total Sections: ${stats.totalSections}`),
      {
        padding: 1,
        margin: 1,
        borderStyle: "round"
...
async function interactiveGenerate(options) {
  const { templateId } = await import_inquirer.default.prompt([
    {
      type: "list",
      name: "templateId",
  ...
  logger2.info(`
${import_chalk2.default.bold("Template:")} ${template.name}`);
  logger2.info(`${import_chalk2.default.gray(template.description)}
`);
  const fields = {};
  for (const field of template.fields) {
    const prompt = {
      name: field.name,
      message: field.label + (field.required ? import_chalk2.default.red(" *") : "")
    };
    switch (field.type) {
      case "text":
      case "email":
      ...
      case "array":
        prompt.type = "input";
        prompt.message += import_chalk2.default.gray(" (comma-separated)");
        prompt.filter = (input) => input.split(",").map((s) => s.trim());
        break;
      default:
        prompt.type = "input";
    }
  ...
  const { outputPath, filename } = await import_inquirer.default.prompt([
    {
      type: "input",
      name: "outputPath",
  ...
  if (options.dryRun) {
    logger2.info("\n" + import_chalk2.default.bold("Preview Mode") + "\n");
  ...
  const spinner = (0, import_ora.default)("Generating file...").start();
  try {
...
\u2713 File generated successfully!`);
      console.log(
        (0, import_boxen.default)(
          import_chalk2.default.white.bold("Generation Summary\n\n") + import_chalk2.default.gray(`File: ${result.filePath}
`) + import_chalk2.default.gray(`Size: ${result.metadata.fileSize} bytes
`) + import_chalk2.default.gray(`Lines: ${result.metadata.lineCount}`),
          {
            padding: 1,
            margin: 1,
            borderStyle: "round",
      ...
      logger2.error("Generation failed:");
      result.errors?.forEach((e) => console.log(`  ${import_chalk2.default.red("\u2022")} ${e}`));
    }
  } catch (error) {
...
async function quickGenerate(options) {
  if (!options.template && !options.category) {
    logger2.error("Please specify either --template or --category, or use --interactive mode");
    process.exit(1);
  }
  const template = options.template ? templateLibrary.getTemplate(options.template) : templateLibrary.getTemplatesByCategory(options.category)[0];
  if (!template) {
    logger2.error("Template not found");
    process.exit(1);
  }
  logger2.info(`Using template: ${template.name}`);
  logger2.error("Quick mode requires a configuration file or use --interactive mode");
  process.exit(1);
}
